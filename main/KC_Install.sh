...
case "${dbtype}" in
	"ORACLE")
        ...
        then
			...
			sqlplus "${un}"/"${pw}${DBSvrNm}" < KRC-RELEASE-3_1_SP2-Upgrade-ORACLE.modified.sql
			sqlplus "${un}"/"${pw}${DBSvrNm}" < KC-RELEASE-3_1_SP2-Upgrade-ORACLE.modified.sql
			if [ "${InstRice}" = 'Y' ]
			then
				sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR-RELEASE-3_1_SP2-Upgrade-ORACLE.modified.sql
			else
				sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR-RELEASE-3_1_SP2-SR-ORACLE.modified.sql
			fi
			mv *.log ../LOGS/
			cd ..

			cd KC-RELEASE-3_1_SP3-SCRIPT
			sqlplus "${un}"/"${pw}${DBSvrNm}" < KRC-RELEASE-3_1_SP3-Upgrade-ORACLE.modified.sql
			sqlplus "${un}"/"${pw}${DBSvrNm}" < KC-RELEASE-3_1_SP3-Upgrade-ORACLE.modified.sql
			if [ "${InstRice}" = 'Y' ]
			then
				sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR-RELEASE-3_1_SP3-Upgrade-ORACLE.modified.sql
			else
				sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR-RELEASE-3_1_SP3-SR-ORACLE.modified.sql
			fi
			mv *.log ../LOGS/
			cd ..

			cd KC-RELEASE-3_1_SP4-SCRIPT
            sqlplus "${un}"/"${pw}${DBSvrNm}" < KRC-RELEASE-3_1_SP4-Upgrade-ORACLE.modified.sql
            sqlplus "${un}"/"${pw}${DBSvrNm}" < KC-RELEASE-3_1_SP4-Upgrade-ORACLE.modified.sql
            if [ "${InstRice}" = "Y" ]
            then
                sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < RICE-1_0_3_1-1_0_3_2-Upgrade-ORACLE.modified.sql
                sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR-RELEASE-3_1_SP4-Upgrade-ORACLE.modified.sql
            else
            	sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR-RELEASE-3_1_SP4-SR-ORACLE.modified.sql
            fi
            mv *.log ../LOGS/
            cd ..

			cd KC-RELEASE-3_1_1-SCRIPT
			sqlplus "${un}"/"${pw}${DBSvrNm}" < KC-RELEASE-3_1_1-Upgrade-ORACLE.modified.sql
			if [ "${InstRice}" = 'Y' ]
			then
				sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR-RELEASE-3_1_1-Upgrade-ORACLE.modified.sql
			else
				sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR-RELEASE-3_1_1-SR-ORACLE.modified.sql
			fi
			mv *.log ../LOGS/
			cd ..
		fi
		...
		then
            ...
            sqlplus "${un}"/"${pw}${DBSvrNm}" < KC-RELEASE-3_2-Upgrade-ORACLE.modified.sql
            if [ "${InstRice}" = 'Y' ]
            then
            	sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR-RELEASE-3_2-Upgrade-ORACLE.modified.sql
            else
            	sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR-RELEASE-3_2-SR-ORACLE.modified.sql
            fi
            mv *.log ../LOGS/
            cd ..
            ...
            if [ "${mode}" = "EMBED" ]
            then
                sqlplus "${un}"/"${pw}${DBSvrNm}" < KRC_RICE-RELEASE-4_0-Upgrade-ORACLE.modified.sql
            fi
            sqlplus "${un}"/"${pw}${DBSvrNm}" < KC_RICE-RELEASE-4_0-Upgrade-ORACLE.modified.sql
            if [ "${InstRice}" = "Y" ]
            then
                sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR_RICE-RELEASE-4_0-Upgrade-ORACLE.modified.sql
                sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR-RELEASE-4_0-Upgrade-ORACLE.modified.sql
            else
            	sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR-RELEASE-4_0-SR-ORACLE.modified.sql
            fi
            sqlplus "${un}"/"${pw}${DBSvrNm}" < KC-RELEASE-4_0-Upgrade-ORACLE.modified.sql
            mv *.log ../LOGS/
            cd ..
            ...
            if [ "${mode}" = "EMBED" ]
            then
                sqlplus "${un}"/"${pw}${DBSvrNm}" < KRC_RICE-RELEASE-5_0-Upgrade-ORACLE.modified.sql
            fi
            if [ "${InstRice}" = "Y" ]
            then
                sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR_RICE-RELEASE-5_0-Upgrade-ORACLE.modified.sql
            fi
            sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR-RELEASE-5_0-Upgrade-ORACLE.modified.sql
            sqlplus "${un}"/"${pw}${DBSvrNm}" < KC-RELEASE-5_0-Upgrade-ORACLE.modified.sql
            mv *.log ../LOGS/
            cd ..
        fi
        ...
        then
            ...
            sqlplus "${un}"/"${pw}${DBSvrNm}" < KC-RELEASE-5_0_1-Upgrade-ORACLE.modified.sql
            sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR-RELEASE-5_0_1-Upgrade-ORACLE.modified.sql
            mv *.log ../LOGS/
            cd ..
        fi
        ...
        then
            ...
            if [ "${mode}" = "EMBED" ]
            then
                sqlplus "${un}"/"${pw}${DBSvrNm}" < KRC_RICE-RELEASE-5_1_0-Upgrade-ORACLE.modified.sql
            fi
            if [ "${InstRice}" = "Y" ]
            then
                sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR_RICE-RELEASE-5_1_0-Upgrade-ORACLE.modified.sql
            fi
            sqlplus "${un}"/"${pw}${DBSvrNm}" < KC-RELEASE-5_1_0-Upgrade-ORACLE.modified.sql
            sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR-RELEASE-5_1_0-Upgrade-ORACLE.modified.sql
            mv *.log ../LOGS/
            cd ..
		fi
        ...
        then
            ...
            if [ "${mode}" = "EMBED" ]
            then
                sqlplus "${un}"/"${pw}${DBSvrNm}" < KRC_RICE-RELEASE-5_1_1-Upgrade-ORACLE.modified.sql
            fi
            if [ "${InstRice}" = "Y" ]
            then
                sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR_RICE-RELEASE-5_1_1-Upgrade-ORACLE.modified.sql
            fi
            sqlplus "${un}"/"${pw}${DBSvrNm}" < KC-RELEASE-5_1_1-Upgrade-ORACLE.modified.sql
            sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR-RELEASE-5_1_1-Upgrade-ORACLE.modified.sql
            mv *.log ../LOGS/
            cd ..
		fi
	...
	if [ "${version}" = "5.1.1" ] || [ "${version}" = "5.1" ] || [ "${version}" = "5.0.1" ] || [ "${version}" = "5.0" ] || [ "${version}" = '3.1.1' ] || [ "${version}" = "NEW" ]
        then
            ...
            if [ "${mode}" = "EMBED" ]
            then
                sqlplus "${un}"/"${pw}${DBSvrNm}" < KRC_RICE-RELEASE-5_2_0-Upgrade-ORACLE.modified.sql
            fi
            if [ "${InstRice}" = "Y" ]
            then
                sqlplus "${Riceun}"/"${Ricepw}${RiceDBSvrNm}" < KR_RICE-RELEASE-5_2_0-Upgrade-ORACLE.modified.sql
            fi
            sqlplus "${un}"/"${pw}${DBSvrNm}" < KC-RELEASE-5_2_0-Upgrade-ORACLE.modified.sql
